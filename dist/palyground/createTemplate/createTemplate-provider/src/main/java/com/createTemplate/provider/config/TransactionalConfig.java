 ...
 * 
 * @author:  
 * @date: 2018年5月28日 下午3:18:26 
 * @version V1.0
 */
@Setter
@Getter
@Configuration
@ConditionalOnExpression("${osc.transactional.enabled:true}")
@ConfigurationProperties(prefix = "spring.transactional")
public class TransactionalConfig {
     ...
     * 可传播事务配置
     */
    private static final String[] DEFAULT_REQUIRED_METHOD_RULE_TRANSACTION_ATTRIBUTES = { "add*", "set*", "save*",
            "insert*", "delete*", "update*", "edit*", "batch*", "space*", "remove*" , "query*" };
    ...
    public BeanNameAutoProxyCreator customizeTransactionBeanNameAutoProxyCreator() {
        ...
        List<String> transactionBeanNames = new ArrayList<>(DEFAULT_TRANSACTION_BEAN_NAMES.length
                + customizeTransactionBeanNames.length);
        // 默认
